...
| Task ID | Task Name | 분류/모듈 | Phase | 담당 | 공수(일) | 선행 태스크 | 주차 | 핵심 OSS(Tier) | 실제 상태 |
|---|---|---|---|---|---|---|---|---|---|
| **T-001** | 저장소 구조 및 Python 가상환경 구축 | 공통/인프라 | Phase A | A | 0.5 | 없음 | 1주차 | - | 완료 |
| **T-002** | Upstage API 연동 및 공통 모듈 구현 | 공통/인프라 | Phase A | A | 0.5 | T-001 | 1주차 | - | 완료 |
| **T-003** | ESP-IDF v5.3+ 개발 환경 구축 | 공통/인프라 | Phase HW | B | 1.0 | T-703 | 10주차 | esp-idf (Tier 1) | 착수전 |
| **T-004** | ESP32-P4 BSP 공식 예제 빌드 및 플래시 검증 | 공통/인프라 | Phase HW | B | 1.5 | T-003 | 10주차 | LVGL (Tier 1), esp-idf (Tier 1), esp-bsp (Tier 1) | 착수전 |
| **T-005** | 로깅 모듈 및 Run ID 기반 산출물 체계 구현 | 공통/인프라 | Phase A | A | 0.5 | T-002 | 1주차 | - | 완료 |
| **T-006** | M5Stack CoreS3 개발 환경 및 예제 검증 | 공통/인프라 | Phase HW(컷) | B | 1.0 | T-003 | 컷(2단계) | - | 착수전(컷) |
| **T-007** | **[Spike]** ESP32-P4 LCD/터치 PSRAM 메모리 최적화 검증 | 스파이크 | Phase HW | B | 1.0 | T-004 | 10주차 | LVGL (Tier 1), esp-idf (Tier 1) | 착수전 |
| **T-008** | **[Spike]** Upstage Document Parse 표 추출 성능 검증 | 스파이크 | Phase A | A | 1.0 | T-002 | 1주차 | - | 완료 |
| **T-009** | **[Spike]** Solar Pro 3 비전 멀티모달 입력 및 판정 실험 | 스파이크 | Phase A | A | 1.0 | T-002 | 1주차 | - | 완료 |
| **T-010** | **[Spike]** NC AI VARCO Art 이미지 생성 API 접속 검증 | 스파이크 | Phase A | A | 1.0 | T-002 | 2주차 | - | 상태불일치 |
| **T-011** | **[Spike]** USB 카메라 LCD 촬영 화질 및 반사 제어 전처리 실험 | 스파이크 | Phase HW | B | 1.0 | T-004 | 11주차 | OpenCV (Tier 1) | 착수전 |
| **T-012** | **[Spike]** 개발·시연 OS 및 Docker/USB/카메라 접근 범위 결정 | 스파이크 | Phase A/HW | A+B | 2.0 | T-001 | 2주차(SW)·11주차(HW) | - | 상태불일치 |
| **T-013** | **[Spike]** HIL 물리 환경 최소 구성 및 시리얼 포트 매핑 검증 | 스파이크 | Phase HW | B | 1.0 | T-011, T-012 | 11주차 | - | 착수전 |
| **T-101** | Typer CLI 명령어 엔트리포인트 구현 | 입력 처리 | Phase A | A | 1.0 | T-005 | 2주차 | Typer (Tier 1) | 완료 |
| **T-102** | 사용자 UI 요구사항 및 PDF 파일 검증 모듈 구현 | 입력 처리 | Phase A | A | 0.5 | T-101 | 2주차 | Typer (Tier 1), Pydantic (Tier 1) | 완료 |
| **T-201** | Upstage Document Parse 데이터 파싱 및 텍스트 청킹 | 문서 이해 | Phase A | A | 1.0 | T-008, T-102 | 3주차 | - | 완료 |
| **T-202** | 데이터시트 핵심 스펙(디스플레이, 레지스터, 터치) 분석기 | 문서 이해 | Phase A | A | 1.5 | T-201 | 3주차 | - | 완료 |
| **T-203** | 기술 지식 베이스(Technology KB) JSON 스키마 변환기 | 문서 이해 | Phase A | A | 1.0 | T-202 | 4주차 | Pydantic (Tier 1) | 완료 |
| **T-301** | 보드 프로필 매퍼 및 BSP 고정 템플릿 연동 모듈 | 코드 생성 | Phase A | B | 1.0 | T-203 | 4주차 | esp-bsp (Tier 1) | 착수전 |
| **T-302** | Solar Pro 3 LVGL UI 레이아웃 프롬프트 설계 | 코드 생성 | Phase A | A | 1.5 | T-203, T-009 | 4주차 | LVGL (Tier 1) | 완료 |
| **T-303** | LLM 생성 코드 위젯 트리 및 이벤트 핸들러 파서 구현 | 코드 생성 | Phase A | A | 1.5 | T-302 | 5주차 | LVGL (Tier 1) | 완료 |
| **T-304** | 에이전트 컨텍스트 토큰 예산 관리 모듈 구현 | 코드 생성 | Phase A | A | 1.0 | T-303 | 5주차 | - | 착수전 |
| **T-401** | VARCO Art API 연동 HMI 이미지 생성기 구현 | 시각 에셋 | Phase A | A | 1.5 | T-010, T-303 | 5주차 | - | 착수전 |
| **T-402** | GUI 이미지 에셋 LVGL C 배열 변환기(lv_img_conv 연동) | 시각 에셋 | Phase A | B | 1.0 | T-401 | 6주차 | LVGL (Tier 1) | 착수전 |
| **T-501** | BoardTarget 추상클래스 설계 및 ESP32-P4 컴파일 서브프로세스 구현 | 빌드/플래시 | Phase HW | B | 1.5 | T-301, T-402, T-703 | 11주차 | LVGL (Tier 1), esp-idf (Tier 1) | 착수전 |
| **T-502** | esptool 연동 타깃 보드 펌웨어 플래시 제어 모듈 | 빌드/플래시 | Phase HW | B | 1.0 | T-501 | 11주차 | esptool (Tier 1) | 착수전 |
| **T-503** | GCC/Clang 컴파일러 에러 로그 구문 분석기 구현 | 빌드/플래시 | Phase A | B | 1.0 | 없음(모의 로그) | 8주차 | - | 착수전 |
| **T-504** | ESP32-S3(M5Stack CoreS3) 빌드 및 플래시 드라이버 포팅 | 빌드/플래시 | Phase HW(컷) | B | 1.0 | T-006, T-502 | 컷(2단계) | esp-bsp (Tier 1), esptool (Tier 1) | 착수전(컷) |
| **T-601** | USB 카메라 기동 및 물리적 촬영 프레임 캡처 모듈 | 실기 검증 | Phase HW | B | 1.0 | T-011, T-502 | 12주차 | OpenCV (Tier 1) | 착수전 |
| **T-602** | 촬영 이미지 원근 보정 및 UI 영역 전처리 모듈 | 실기 검증 | Phase HW | B | 1.5 | T-601 | 12주차 | OpenCV (Tier 1) | 착수전 |
| **T-603** | OpenCV 기반 위젯 크기/위치 정량 PASS/FAIL 판정기 | Vision(시뮬+실기) | Phase A | B | 1.5 | T-802, T-303 | 6주차 | lv_port_pc_vscode (Tier 1), OpenCV (Tier 1) | 착수전 |
| **T-604** | OCR 및 Solar Pro 3 비전 기반 텍스트/의미 일치 분석기 | Vision(시뮬+실기) | Phase A | B | 2.0 | T-603, T-009 | 7주차 | EasyOCR (Tier 1) | 착수전 |
| **T-701** | LangGraph 2단계 상태머신 (Sim + HW) 그래프 구축 | 오케스트레이션 | Phase A | A | 1.5 | T-304, T-802, T-603, T-604 | 7주차 | LangGraph (Tier 1) | 착수전 |
| **T-702** | 자가 수정 루프 재진입 및 라운드 제어기 (sim 5회 / hw 2회) | 오케스트레이션 | Phase A | A | 1.5 | T-701 | 8주차 | LangGraph (Tier 1) | 착수전 |
| **T-703** | 체크포인트 세션 저장소 및 최종 검증 보고서 생성기 | 오케스트레이션 | Phase A | A | 1.0 | T-702 | 8주차 | LangGraph (Tier 1) | 착수전 |
| **T-801** | LVGL PC VSCode 시뮬레이터(SDL2) 환경 연동 모듈 | 시뮬레이터 | Phase A | B | 1.5 | T-001 | 1주차 | LVGL (Tier 1), lv_port_pc_vscode (Tier 1) | 완료 |
| **T-802** | PC 시뮬레이터 드라이버·스크린샷 캡처 및 자동 구동 | 시뮬레이터 | Phase A | B | 1.0 | T-801 | 2주차 | lv_port_pc_vscode (Tier 1) | 완료 |
| **T-850** | Emscripten WebAssembly 기반 브라우저 렌더링 파이프라인 | 웹 대시보드 | Phase B | B | 2.0 | T-802, M2통과 | 10주차(선택) | LVGL (Tier 1), Emscripten (Tier 3) | 착수전 |
| **T-851** | 에이전트 통합 제어용 FastAPI 백엔드 서버 구축 | 웹 대시보드 | Phase B | A | 1.5 | T-703 | 10주차(선택) | FastAPI (Tier 3) | 착수전 |
| **T-852** | Streamlit/React 기반 UI 대시보드 및 웹 HMI 캔버스 프론트엔드 | 웹 대시보드 | Phase B | A | 2.0 | T-850, T-851 | 11주차(선택) | FastAPI (Tier 3), Streamlit (Tier 3) | 착수전 |
| **T-901** | Typer CLI 기반 시뮬 E2E 파이프라인 통합 테스트 (`--mode sim`) | 통합/시연 | Phase A | A | 1.5 | T-703 | 9주차 | Typer (Tier 1) | 착수전 |
| **T-902** | 평가 지표 4종 자동 정량 측정 (sim 9주 / hw 12주) | 통합/시연 | Phase A/HW | A | 1.5 (sim 1.0 / hw 0.5) | T-901 | 9주차·12주차 | - | 착수전 |
| **T-903** | 파이프라인 극한 테스트(예외 입력, 타임아웃, 케이블 단선) | 통합/시연 | Phase HW | A+B | 2.0 (페어) | T-901, T-502 | 12주차 | esptool (Tier 1) | 착수전 |
| **T-904** | README 문서, 설치 스크립트 작성 및 비원 개발자 검증 | 통합/시연 | Phase A | B | 1.0 | T-802 | 8주차 | - | 착수전 |
| **T-905** | 시연 부스 설치용 하드웨어 물리 결선 및 최종 시연 리허설 | 통합/시연 | Phase HW | B | 1.5 | T-903, T-904 | 12주차 | - | 착수전 |
...
### 2.3 구현 현황 스냅샷 및 실제 착수 순서

> 본 절은 `단위구현계획서_진행현황_재정렬_프롬프트.md` 임무에 따라, 제5장 45개 Task의 실제 구현 상태를 증거(main 커밋 + 산출물 파일 + 검증기록 내용) 기반으로 감사한 결과다. Phase/마일스톤/주차 구조(제1·3·4장)는 변경하지 않았다.
> 판정 기준: (a) `git log main`에 `[T-xxx]` 커밋 존재, (b) 카드 9항 산출물 파일 존재, (c) 카드 13항 검증기록이 PASS(또는 정직한 조건부 PASS)를 보임 — 3가지 모두 충족 시에만 `완료`. 부분 충족은 `진행중`, 근거 없음은 `착수전`, main 기록과 검증기록이 충돌하면 `상태불일치`.
> 증거 수집 시각 기준 브랜치: `feature/T-303-code-parser`. `gh` CLI가 이 머신에 미설치되어 이슈 `status:*` 라벨은 명령으로 검증하지 못했다(라벨 축은 확인불가로 남김).

#### 2.3.1 Task별 실제 상태 감사표 (45개 전수)

| Task ID | 계획서상 주차 | 판정 상태 | 근거(커밋/PR/파일/라벨) |
|---|---|---|---|
| T-001 | 1주차 | 완료 | main `1dc46bc` (PR #72); `.gitignore`·`requirements.txt` 존재; `docs/verification/T-001_env_setup.txt` pytest 5 passed. 라벨 확인불가(gh 미설치) |
| T-002 | 1주차 | 완료 | main `81624fc` (PR #73); `src/common/upstage_client.py`·`.env.example` 존재; `T-002_api_test.txt` 4 passed + 라이브 200 확인 |
| T-003 | 10주차 | 착수전 | main 커밋 없음; 산출물/검증기록 없음 |
| T-004 | 10주차 | 착수전 | main 커밋 없음; 산출물/검증기록 없음 |
| T-005 | 1주차 | 완료 | main `1c8c528` (PR #74); `src/common/logger.py`·`run_manager.py` 존재; `T-005_run_manager_test.txt` 8 passed |
| T-006 | 컷(2단계) | 착수전(컷) | 기본 컷 대상; main 커밋 없음 |
| T-007 | 10주차 | 착수전 | main 커밋 없음; 실기 보드 필요 |
| T-008 | 1주차 | 완료 | main `a6655e1` (PR #75); `tests/test_document_parser.py` 존재; `T-008_extracted_tables.md` 손실율 1.18% GO(6 passed) |
| T-009 | 1주차 | 완료 | main `0e1bf7b` (PR #76); `tests/test_solar_vision.py` 존재; `T-009_mm_eval.txt` 가정 기각(정직한 No-Go=스파이크 완료), OpenCV fallback 실증 9 passed |
| T-010 | 2주차 | 상태불일치 | main `23689a5` (PR #80)·`src/agent/varco_art.py`·`T-010_varco_art_note.md` 존재하나, 검증기록이 라이브 200/201 **미실증(Pending)** + Placeholder Fallback만 실증(DoD 1항 미충족). 완료로 단정 불가 → 사용자 확인 필요 |
| T-011 | 11주차 | 착수전 | main 커밋 없음; 실기 카메라 필요 |
| T-012 | 2주차(SW)·11주차(HW) | 상태불일치 | main `e8a1080` (PR #85)·`docs/environment_decision.md`·`T-012_environment_matrix.md`·`tests/test_env_os_docker.py` 모두 main에 tracked. 그러나 검증기록이 **SW부분 조건부 PASS**(Docker/WSL2/Linux 미검증)이고 HW부분(카메라/USB/시리얼)+DoD 3항은 11주차 이관·미체크. 완료로 단정 불가 → 사용자 확인 필요 |
| T-013 | 11주차 | 착수전 | main 커밋 없음; 선행 T-011/T-012 상태 미확정 |
| T-101 | 2주차 | 완료 | main `0186402` (PR #82); `src/cli/main.py`·`__init__.py` 존재; `T-101_cli_help.txt`+`T-101_pytest.txt` 11 passed |
| T-102 | 2주차 | 완료 | main `216fb86` (PR #84); `src/cli/validator.py` 존재; `T-102_validator_test.txt` 10 passed |
| T-201 | 3주차 | 완료 | main `6bb8a8d` (PR #89); `src/parser/document_parser.py` 존재; `T-201_chunk_output.json` 334청크 size limit 준수·overlap 관측 |
| T-202 | 3주차 | 완료 | main `93a4245` (PR #93); `src/parser/spec_extractor.py`·`prompts.py` 존재; `T-202_spec_result.json` 라이브 5필드 추출(11 passed) |
| T-203 | 4주차 | 완료 | main `53f2934` (PR #95); `src/common/schema.py`·`src/parser/kb_converter.py` 존재; `T-203_kb_schema.json` 규격 JSON |
| T-301 | 4주차 | 착수전 | main 커밋 없음; 산출물(`board_profile_mapper.py`) 없음. 선행 T-203 완료 → 착수 가능 |
| T-302 | 4주차 | 완료 | main `95d9cbc` (PR #97); `src/agent/prompts/layout_generator.py` 존재; `T-302_prompt_draft.md` 12 passed(라이브 포함) |
| T-303 | 5주차 | 완료 | main `dbb9f24` (PR #100, Closes #46); `src/agent/code_parser.py`·`docs/verification/T-303_parsed_tree.json` 존재; `tests/test_code_parser.py` 9 passed + 전체 회귀 127 passed·5 skipped. Pass 2 리뷰 통과 |
| T-304 | 5주차 | 착수전 | main 커밋 없음; 선행 T-303 완료 → 착수 가능 |
| T-401 | 5주차 | 착수전 | main 커밋 없음; 선행 T-303=완료·T-010=상태불일치(확정 필요) |
| T-402 | 6주차 | 착수전 | main 커밋 없음; 선행 T-401 미착수 |
| T-501 | 11주차 | 착수전 | main 커밋 없음; Phase HW |
| T-502 | 11주차 | 착수전 | main 커밋 없음; Phase HW |
| T-503 | 8주차 | 착수전 | main 커밋 없음(모의 로그 fixture 사용, 선행 없음이나 미착수) |
| T-504 | 컷(2단계) | 착수전(컷) | 기본 컷 대상; main 커밋 없음 |
| T-601 | 12주차 | 착수전 | main 커밋 없음; 실기 카메라 필요 |
| T-602 | 12주차 | 착수전 | main 커밋 없음; Phase HW |
| T-603 | 6주차 | 착수전 | main 커밋 없음; 선행 T-802=완료·T-303=완료 → 착수 가능 |
| T-604 | 7주차 | 착수전 | main 커밋 없음; 선행 T-603 미착수 |
| T-701 | 7주차 | 착수전 | main 커밋 없음; 선행 T-304/T-603/T-604 미착수 |
| T-702 | 8주차 | 착수전 | main 커밋 없음; 선행 T-701 미착수 |
| T-703 | 8주차 | 착수전 | main 커밋 없음; 선행 T-702 미착수 |
| T-801 | 1주차 | 완료 | main `9753f64`/`6ec2b30` (PR #77/#79); `src/simulator/CMakeLists.txt`·`main.c` 존재; `T-801_build_check.txt` 실빌드·exe 생성(자동화 세션 육안한계 명시된 조건부 완료) |
| T-802 | 2주차 | 완료 | main `29a15b9` (PR #88); `src/simulator/sim_driver.py` 존재; `T-802_process_kill.txt` 9 passed·고스트 정리 실증(캡처 seam 한계 명시) |
| T-850 | 10주차(선택) | 착수전 | main 커밋 없음; Phase B, M2 통과 전제 |
| T-851 | 10주차(선택) | 착수전 | main 커밋 없음; Phase B |
| T-852 | 11주차(선택) | 착수전 | main 커밋 없음; Phase B |
| T-901 | 9주차 | 착수전 | main 커밋 없음; 선행 T-703 미착수 |
| T-902 | 9주차·12주차 | 착수전 | main 커밋 없음; 선행 T-901 미착수 |
| T-903 | 12주차 | 착수전 | main 커밋 없음; Phase HW 페어 |
| T-904 | 8주차 | 착수전 | main 커밋 없음; 선행 T-802 완료이나 미착수 |
| T-905 | 12주차 | 착수전 | main 커밋 없음; Phase HW 최종 |

**판정 집계**: 완료 14개(T-001·002·005·008·009·101·102·201·202·203·302·303·801·802), 진행중 0개, 상태불일치 2개(T-010·T-012), 착수전 29개(컷 T-006·T-504 포함). 합계 45개.

**WIP=1 규칙 점검**: T-303이 main에 머지(PR #100)되어 현재 `status:in-progress` Task는 0개다. `.cursor/rules/task-workflow.mdc` 0항(WIP=1) 위배 없음. T-010/T-012는 상태불일치로 순서 계산에서 분리했다.

#### 2.3.2 실제 다음 착수 순서 재계산

제3장 의존성 그래프를 기준으로 "완료" 판정 Task만 끝난 선행으로 인정한다. 계획서상 주차가 빨라도 선행이 미완료면 착수 가능이 아니다.

| 순번 | Task ID | 상태 | 선행 완료 여부(선행 ID별) | 착수 가능 여부 |
|---|---|---|---|---|
| 1 | T-304 | 착수전 | T-303=완료 | 착수 가능(5주차) |
| 2 | T-603 | 착수전 | T-802=완료, T-303=완료 | 착수 가능(6주차) |
| 3 | T-301 | 착수전 | T-203=완료 | 착수 가능(4주차) |
| 4 | T-904 | 착수전 | T-802=완료 | 착수 가능(Phase A 독립, 8주차) |
| 5 | T-402 | 착수전 | T-401=착수전 | 불가(T-401 선행) |
| 6 | T-401 | 착수전 | T-010=상태불일치, T-303=완료 | 불가(T-010 확정 필요) |
| 7 | T-604 | 착수전 | T-603=착수전, T-009=완료 | 불가(T-603 선행) |
| - | T-701~703, T-901~902 | 착수전 | 상위 Phase A 선행 미완료 | 불가 |
| - | Phase HW(T-003·004·007·011·013·501·502·601·602·903·905), Phase B(T-850~852) | 착수전 | 선행 미완료 / M2 미통과 | 불가 |

*타이브레이커*: 착수 가능 Task가 복수면 기존 계획서 주차·Phase 순서를 1차 기준으로 사용한다(구조 불변 원칙 준수). T-303 머지 완료 후 현재 착수 가능한 것은 T-304(5주차)·T-603(6주차)·T-301(4주차)·T-904(8주차)이며, 주차 순 타이브레이커로는 T-301(4주차) 또는 계획 흐름상 다음 순서인 T-304(5주차)가 우선이다. 규칙상 WIP=1이므로 한 번에 한 개만 진행한다.

#### 2.3.3 사용자 확인 필요 (상태불일치)

아래 2개는 임의로 완료/미완료를 확정하지 않고 사용자 판단에 넘긴다. 순서 계산에서 제외했다.

| Task ID | 충돌 내용 | 확인해야 할 것 |
|---|---|---|
| T-010 | main 머지·산출물·검증기록은 존재하나, 검증기록이 라이브 200/201을 실증하지 못하고 Placeholder Fallback만 확인(DoD 1항 미충족을 정직하게 기록). "머지됨=완료"와 "DoD 미충족" 충돌 | NC_VARCO_API_KEY 확보 후 라이브 재검증하여 완료로 승격할지, 스파이크 조건부 완료로 인정할지 |
| T-012 | 산출물이 이미 main에 tracked이며 SW부분 조건부 PASS. 그러나 HW부분(카메라/USB/시리얼)·DoD 3항은 11주차로 이관·미체크. 재정렬 프롬프트가 지적한 "브랜치 미커밋 산출물" 전제는 stale(파일은 main에 존재) | SW부분만으로 "완료"로 볼지, HW부분까지 포함해 11주차까지 "진행중"으로 유지할지 |
...
* **11. 완료 판정(DoD)**:
  - [x] 가상환경 정상 설치 및 활성화 성공.
  - [x] requirements.txt에 명시된 필수 패키지 설치 완료.
  - [x] .gitignore 파일이 루트 디렉토리에 정상 반영됨.
  <!-- 근거: PR #72(1dc46bc), docs/verification/T-001_env_setup.txt pytest 5 passed -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] 환경변수 로더가 `.env` 파일을 정상 파싱함.
  - [x] Upstage API 클라이언트가 인스턴스화 시 예외를 발생시키지 않음.
  - [x] 테스트를 통해 기본적인 API 통신 성공 확인.
  <!-- 근거: PR #73(81624fc), docs/verification/T-002_api_test.txt 4 passed + 라이브 Solar 200 확인 -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] Run ID 생성 시 중복성이 없고 고유성이 확인됨.
  - [x] 하위 폴더 3종(logs, assets, build)이 예외 발생 없이 OS 폴더 상에 물리 생성됨.
  <!-- 근거: PR #74(1c8c528), docs/verification/T-005_run_manager_test.txt 8 passed -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] Upstage API 응답 상태코드 200 회신 완료.
  - [x] 데이터시트 내 주요 데이터가 마크다운 텍스트 상에 정렬 구조를 유지한 채 안착됨.
  <!-- 근거: PR #75(a6655e1), docs/verification/T-008_extracted_tables.md 셀 손실율 1.18% GO, 6 passed -->
* **12. 실패 시 대처**:
...
* **7. 목적**: **[가정: Solar Pro 3가 멀티모달 비전 인풋을 정식 지원하고, LCD 캡처 이미지 상의 텍스트 및 UI 구조를 인식할 수 있는가]**를 실측 검증하여, 보조 판정 수단으로서의 가용 여부를 진단한다.
  * **[스파이크 결론]** 실측 결과 **가정 [1] 기각**: Solar Pro 3/Solar Pro 2는 채팅 엔드포인트에서 이미지 멀티모달 입력을 미지원(실제 API가 `HTTP 400 "Image input is not allowed for this model"`로 명시적 거부). 12항 대처에 따라 OpenCV 픽셀 매칭 + 로컬 OCR 100% 경로를 메인 판정으로 확정하고, Solar Pro 3는 텍스트 기반 자가 치유용으로 범위를 축소한다.
* **8. 구현 내용**:
...
* **11. 완료 판정(DoD)**:
  - [x] Solar Pro 3/2의 이미지 멀티모달 입력 지원 여부를 실제 API 호출로 진단 완료 (결과: **미지원**, `HTTP 400 "Image input is not allowed for this model"` — REST·langchain 양쪽 동일, 텍스트 전용 대조군은 200 정상).
  - [x] 이미지 판정 Fallback(OpenCV 픽셀 매칭 + 로컬 OCR 100% 경로)이 동작함을 실증하고(`ui_misplaced.png`→FAIL, `ui_normal.png`→PASS), 후속 Task(T-604 로컬 OCR 메인 고정, T-302 텍스트 전용이라 영향 없음) 영향을 문서화함.
  <!-- 근거: PR #76(0e1bf7b), docs/verification/T-009_mm_eval.txt 가정 기각(정직한 No-Go=스파이크 완료), 9 passed -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] Typer 기반 CLI 쉘 엔트리 구조 구축 완료.
  - [x] 지정 매개변수 유효성 검사 루틴 반영.
  <!-- 근거: PR #82(0186402), docs/verification/T-101_cli_help.txt + T-101_pytest.txt 11 passed -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] 헤더 바이트 검사로 확장자 위조 파일 검증 완료.
  - [x] 빈 요구사항 텍스트 파일에 대한 차단 예외 처리 동작 성공.
  <!-- 근거: PR #84(216fb86), docs/verification/T-102_validator_test.txt 10 passed -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] PDF가 Upstage API를 통과해 텍스트 블록 리스트로 분할 완료됨.
  - [x] 누락되거나 중첩이 없는 MECE 조건의 청크 셋 완성.
  <!-- 근거: PR #89(6bb8a8d), docs/verification/T-201_chunk_output.json 334청크 size limit 준수·overlap 관측 -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] 추출 알고리즘이 필수 하드웨어 스펙 필드 5종을 정상적으로 검출 완료함.
  - [x] 오검출이나 누락 발생 시 빈 문자열 또는 None을 처리하는 안전 로직 구현.
  <!-- 근거: PR #93(93a4245), docs/verification/T-202_spec_result.json 라이브 5필드 추출, 11 passed -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] Pydantic 스키마 정의 및 Validation 절차 구현 완료.
  - [x] 변환된 JSON 파일 형상이 `tests/data/expected_kb.json`과 구조적으로 100% 동등함.
  <!-- 근거: PR #95(53f2934), docs/verification/T-203_kb_schema.json 규격 JSON 생성 -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] 프롬프트 템플릿 내에 1024x600 해상도 제약 조건이 런타임에 동적 매핑되도록 처리함.
  - [x] 불필요한 C언어 main 함수나 헤더 중복 작성을 배제하고 `ui_screens.c`에 특화된 코드만 출력하도록 시스템 롤 부여 완료.
  <!-- 근거: PR #97(95d9cbc), docs/verification/T-302_prompt_draft.md 12 passed(라이브 Solar C코드 생성 포함) -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] 생성 C 코드 내 위젯 목록 100% 인식 완료.
  - [x] 부모-자식 트리 상속 위계 구조 복원 성공.
  <!-- 근거: PR #100(dbb9f24), Closes #46, docs/verification/T-303_parsed_tree.json 위젯 트리 덤프, tests/test_code_parser.py 9 passed + 전체 127 passed·5 skipped -->
...
* **11. 완료 판정(DoD)**:
  - [x] SDL2 개발 환경 및 CMake 연동 빌드 구성 완료.
  - [x] 생성된 LVGL C 코드와 링킹되어 정상 화면이 PC 화면상에 팝업됨.
  - [x] 마우스 클릭 등의 터치 이벤트 모방 동작 작동 확인.
  <!-- 근거: PR #77/#79(9753f64/6ec2b30), docs/verification/T-801_build_check.txt 실빌드·exe 생성(자동화 세션 육안 상호작용 한계는 기록에 명시된 조건부 완료) -->
* **12. 실패 시 대처**:
...
* **11. 완료 판정(DoD)**:
  - [x] 백그라운드 PC 컴파일 제어 클래스 구현 완료.
  - [x] 시뮬레이터 프로세스의 정확한 Life-cycle 제어(기동 -> 자동 종료) 검증 완료.
  <!-- 근거: PR #88(29a15b9), docs/verification/T-802_process_kill.txt 9 passed·고스트 정리 실증(스크린샷 캡처는 test seam으로 검증, 라이브 SDL 창 캡처 한계 명시된 조건부 완료) -->
* **12. 실패 시 대처**:
